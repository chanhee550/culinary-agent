...
async def scan(files: list[UploadFile] = File(...)):
    """다중 이미지 업로드 → confirmed/unknowns 분리 반환."""
    if not files:
...
def create_ingredient(item: IngredientIn):
    result = add_ingredient(item.name, item.category, item.quantity, source="manual")
    return IngredientOut(
        id=result.id, name=result.name, category=result.category,
        quantity=result.quantity, source=result.source,
    )
...
def patch_ingredient(ingredient_id: str, body: IngredientUpdate):
    update_ingredient(
        ingredient_id,
        name=body.name,
...
def recipes(req: RecipeRequest):
    """보유 재료 기반 레시피 추천. ingredients 미지정 시 DB에서 자동 로드."""
    ingredients = req.ingredients or get_ingredient_names()
    if not ingredients:
        return []
    return recommend_recipes(ingredients, max_missing=req.max_missing)
